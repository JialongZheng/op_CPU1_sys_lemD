...
void StateMachineProc()
{
   ...
   // 	    *(&(AdcObj.adChZero->A0ZERO)+ch) = 0;
    ...
    if((RunData.StateMachine.FaultCode0Reg.all)||(RunData.StateMachine.FaultCode1Reg.all)||(RunData.StateMachine.FaultCode2Reg.all)||(RunData.StateMachine.FaultCode3Reg.all))
    {
	...
	switch(RunData.StateMachine.StateMachine)
	    {
	     ...
	     if((RunData.InvControlOutput.GridVda > 100)&&(RunData.InvControlOutput.GridVdb > 100)&&(RunData.InvControlOutput.GridVdc > 100))
	     {
	       ...
	       if(RunData.StateMachine.MswReg.bits.Fault)
	        ...
	        else
	        {
	            Inv_controlobj.CtlINVMode = VF_ISLAND;
	             ...
	             if((RunData.StateMachine.MswReg.bits.Start) && (NF_Data.FltTrip==1))
	             {
	                NF_Data.FltTrip = 0;
	           //     NF_Data.Step = 0;
	       ...
	       //         NF_Data.OpNum = NF_Write_Wave;
	             ...
	             if((NF_Data.FltRdEn==0)&&(NF_Data.OpNum==0)&&(RunData.StateMachine.MswReg.bits.FltTrip==1))
	             {
	                 NF_Data.Step = 0;
	                 NF_Data.Type = RD_Fault;
	                 NF_Data.OpNum = NF_Write_Wave;
